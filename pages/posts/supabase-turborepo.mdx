---
title: Supabase <3 Turborepo
date: 2022/12/06
description: A guide to setting up a pnpm monorepo with Supabase and Turborepo
author: psteinroe
image: https://philipp.steinroetter.com/api/og?title=Supabase <3 Turborepo
---
...
The script first changes into the root directory before running the type generation. All types are written into a `database.ts` file of the same package. Finally, the generated types are formattedusing prettier.

You might have noticed that the `--local` flag is used. To ensure you have a Supabase intance running, configure the `generate` task similar to the `test` and `lint` tasks within the `turbo.json` file:
...
If you now run `pnpm run generate`, the types will be generated using a locally started Supabase instances.
